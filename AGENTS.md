...
- **Write posts in Markdown.** Each post is `blog/posts/<slug>.md` with YAML front matter (`title`, `date`, `excerpt`). The identical HTML shell at `blog/<slug>/index.html` (copy of `blog/_template/`) loads that file in the browser and renders it. `blog/posts.json` is the published slug list (newest first); the listing, homepage `#blog` section, and jump-to palette read titles/dates from front matter.
